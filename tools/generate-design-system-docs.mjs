#!/usr/bin/env node
/**
 * Generate the source-verified, per-component API reference for MeoUI.
 *
 * Output is portable by default: write into the repository's docs tree, or set
 * MEOUI_DESIGN_SYSTEM_DIR when an external documentation workspace is desired.
 * The bilingual editorial documentation in 01–04 is human maintained and is
...
const output = path.resolve(
    process.env.MEOUI_DESIGN_SYSTEM_DIR
    || path.join(root, "docs", "design-system"),
);
...
if (!sourcePaths.length) {
    throw new Error("No exported Meo QML types found in CMakeLists.txt.");
}

// Safety boundary: --clean may only replace generated component API files.
// It must never remove the output root or its human-authored bilingual guides.
if (clean && fs.existsSync(apiRoot)) {
...
function apiFor(file) {
    ...
    const properties = [...text.matchAll(/^\s*(readonly\s+)?property\s+([^\s]+)\s+(\w+)(?:\s*:\s*([^\n/]+))?/gm)]
        .map((match) => ({
            name: match[3],
            type: `${match[1] ? "readonly " : ""}${match[2]}`,
            value: (match[4] || "").trim(),
        }));
    const signals = [...text.matchAll(/^\s*signal\s+(\w+\([^)]*\))/gm)]
        .map((match) => match[1]);
...
for (const component of components) {
    const group = grouped.get(component.categoryEn) || [];
    group.push(component);
    grouped.set(component.categoryEn, group);
}

const groups = [...grouped.entries()].sort(([a], [b]) => a.localeCompare(b));
const indexZh = groups.map(([, entries]) =>
    `## ${entries[0].categoryZh}\n\n${entries.map(({ name }) => `- [[api/${name}|${name}]] | [[api/${name}.en|English]]`).join("\n")}`,
).join("\n\n");
const indexEn = groups.map(([category, entries]) =>
    `## ${category}\n\n${entries.map(({ name }) => `- [[api/${name}|中文]] | [[api/${name}.en|${name}]]`).join("\n")}`,
).join("\n\n");

const generatorPath = "tools/generate-design-system-docs.mjs";
...
${component.properties.map(({ name, type, value }) => `| \`${name}\` | \`${type}\` | ${value ? `\`${value.replaceAll("|", "\\|")}\`` : "—"} |`).join("\n")}`
    ...
    const signals = component.signals.length
        ? component.signals.map((signal) => `- \`${signal}\``).join("\n")
        : "此文件没有声明自定义 QML 信号。\n\nNo custom QML signals are declared in this file.";

    // Keep source references repository-relative. Generated docs may live in a
    // different workspace, so embedding a file:// URL would leak the build
    // machine's absolute checkout path and make the document non-portable.
    const common = `- 类别 / Category: ${component.categoryZh} / ${component.categoryEn}
- 源码 / Source: \`${component.file}\`
