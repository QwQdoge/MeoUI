    ...
    readonly property var categories: [
        ...
        },
        {
            ...
            "components": [
                ...
                component("MeoStatusStrip", "components/MeoStatusStrip.qml", "Backend-agnostic compact system-status row driven by semantic status maps.", "connected, active, attention, text-hidden, unavailable", "Five configurations, accessible status summary", "statusModel, active, showText, iconSize, textScale", "MeoStatusStrip { statusModel: [{ id: \"wifi\", iconName: \"wifi\", active: true }] }"),
        ...
        },
        {
            ...
            "components": [
                ...
                component("MeoMotionSurface", "components/MeoMotionSurface.qml", "Elevated surface primitive with dynamic color, shape, shadow, reusable spring entrance motion on either axis, and an explicit outline policy.", "outlined default, borderless tonal, elevated, translucent, dynamic color, horizontal reveal, vertical popout reveal", "Resting, borderless tonal, palette transition, revealed", "type, interactive, bouncy, surfaceStyle, showOutline, color, radius, elevation, entranceAxis, entranceDistance, reveal()", "MeoMotionSurface { surfaceStyle: \"tonal\"; entranceAxis: \"y\"; showOutline: false; animateOnCompleted: true }"),
                component("MeoInteractionMotion", "components/MeoInteractionMotion.qml", "Interruptible hover, press, and active motion shared by controls, cards, toolbar triggers, and shell surfaces. Disabled or reduced spatial motion resolves to neutral scale and offsets instead of jumping instantly.", "rest, hover, pressed, active, disabled motion, reduced motion", "Rest, hovered, pressed, active, neutral-disabled, retargeted in flight", "hovered, pressed, active, motionEnabled, spatialMotionAllowed, motionProfile, speed, offsetXTarget, liftMultiplier, targetScale, targetOffsetX, targetOffsetY, resolvedScale, resolvedOffsetX, resolvedOffsetY, snapToCurrentState()", "MeoInteractionMotion { hovered: button.hovered; pressed: button.down; active: selected }"),
                component("MeoRevealMotion", "components/MeoRevealMotion.qml", "Interruptible open/closed reveal state shared by popouts, panes, cards, and shell surfaces. Supports two-dimensional spatial travel plus scale, including lazy hosts instantiated already open.", "active, inactive, reduced motion, custom scale, X/Y travel, lazy-host entrance", "Open, closed, diagonal reveal, retargeted in flight, first-mount reveal, reduced motion", "active, motionEnabled, animateOnCompleted, spatialMotionAllowed, resolvedOpenState, motionProfile, speed, openScale, closedScale, openOffset, closedOffset, openOffsetX, closedOffsetX, openOffsetY, closedOffsetY, resolvedScale, resolvedOffset, resolvedOffsetX, resolvedOffsetY, snapToActiveState(), revealFromClosed()", "MeoRevealMotion { active: panel.opened; animateOnCompleted: true; closedOffsetX: 6; closedOffsetY: -MeoMotion.popupOffset(\"pixel\") * MeoTheme.globalScale }"),
                component("MeoSpringValue", "components/MeoSpringValue.qml", "Interruptible analytic spring value that preserves velocity when a gesture changes target and can seed a sampled value without breaking a bound target.", "standard, expressive, retargeted, reduced motion", "Running, settled, retargeted in flight", "value, targetValue, velocity, spring, enabled, running, snapTo(), snapValue()", "MeoSpringValue { value: 1; targetValue: pressed ? 0.92 : 1 }"),
        ...
        },
        {
            ...
            "components": [
                component("MeoBanner", "components/MeoBanner.qml", "MeoUI inline alert primitive with semantic message and actions.", "tonal, warning, success, error, action, title", "Tonal, warning, success, error, two actions, title-only", "title, text, icon, tone, confirmText, cancelText", "MeoBanner { text: \"Network restored\"; icon: \"info\" }"),
